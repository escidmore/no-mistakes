...
|---|---|---|---|---|---|
| **PR** (create/update) | `gh` CLI, authenticated | `glab` CLI, authenticated | `forgejo-axi`, configured token | `NO_MISTAKES_BITBUCKET_EMAIL` + `NO_MISTAKES_BITBUCKET_API_TOKEN` | `az` CLI + `azure-devops` extension, authenticated |
...
Install the released [`forgejo-axi`](https://github.com/escidmore/forgejo-axi) package (Node.js 20 or newer):

```sh
npm install -g forgejo-axi
```

Give the daemon a Forgejo token through either the generic `FORGEJO_TOKEN` variable or forgejo-axi's host-scoped token variable. For an arbitrary self-hosted hostname, non-default port, or path prefix, also set `FORGEJO_BASE_URL` to the exact web base, for example `https://forge.example:3443/git`. The [environment reference](/no-mistakes/reference/environment/#forgejo_base_url) owns the variable and host-key details. Set [`forgejo_axi_path`](/no-mistakes/reference/global-config/#forgejo_axi_path) when the executable is not on `PATH`.

Verify the connection without mutation:
...
- PR creation, update, exact-head mergeability, and merged-state proof
- Commit-status and required-context polling on supported Forgejo 15 and 16 hosts
- Failed Actions job logs on hosts that advertise the run, run-jobs, and job-log routes; no-mistakes accepts only the native check's canonical run target and verifies the exact PR head before giving logs to the repair agent

Commit-status gating is independent from Actions logs. A Forgejo 15 host, a host without those optional routes, an external status target, or an unavailable log still reports the real check failure; the repair agent proceeds from the failing-check names without logs. A configured repository-scoped token does not need `read:user` merely to make forgejo-axi's separate user probe report authenticated.
