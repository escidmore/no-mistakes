...
Tokens remain in the subprocess environment: no-mistakes never puts a token value in command arguments and redacts Forgejo token values from surfaced provider errors. A repository-scoped token can remain usable when forgejo-axi reports `authenticated: false` because that bit reflects a separate `/user` probe requiring `read:user`; repository commands remain the authority on repository access.
