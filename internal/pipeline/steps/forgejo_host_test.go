...
func TestValidateMergedState_RequiresProofForExpectedHead(t *testing.T) {
	...
	err := validateMergedState(context.Background(), host, pr, "expected")
	if !errors.Is(err, scm.ErrHeadChanged) {
		t.Fatalf("validateMergedState() error = %v, want ErrHeadChanged", err)
	}
}

func TestValidateMergedState_RejectsIncompleteProof(t *testing.T) {
	pr := &scm.PR{Number: "42", URL: "https://forge.example/octo/widgets/pulls/42"}
	host := &mergedProofTestHost{proof: scm.MergedProof{Number: "42", URL: pr.URL, HeadSHA: "expected"}}
	if err := validateMergedState(context.Background(), host, pr, "expected"); err == nil {
		t.Fatal("validateMergedState() error = nil, want unmerged proof rejection")
	}
}
