...
func (h *Host) Available(ctx context.Context) error {
	...
	// forgejo-axi's authenticated bit reflects its /user probe. A repository-
	// scoped token without read:user can be fully valid for PR, status, and run
	// operations while that probe reports false. Requiring a configured token
	// source keeps anonymous/missing credentials distinct; repository commands
	// themselves remain the authority on repository access.
	if !response.Auth.Configured {
		return errors.New("forgejo-axi has no configured credential for the configured Forgejo host; configure a host-scoped token")
	}
...
func (h *Host) readChecks(ctx context.Context, pr *scm.PR) (checksResult, error) {
	...
	if _, err := h.normalizeChecks(response.Checks); err != nil {
		return checksResult{}, err
	}
...
func (h *Host) normalizeChecks(result checksResult) ([]scm.Check, error) {
	...
	if len(checks) == 0 {
		checks = append(checks, scm.Check{Name: "Forgejo: no commit statuses reported", Bucket: scm.CheckBucketPending})
	}
	return checks, nil
}
